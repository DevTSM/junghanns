...
class DeclineTrasnfersProduct extends StatefulWidget {
  final Function(String) onReject;

  DeclineTrasnfersProduct({required this.onReject, super.key});

  @override
  State<StatefulWidget> createState() => _DeclineTrasnfersProductState();
}

class _DeclineTrasnfersProductState extends State<DeclineTrasnfersProduct> {
  ...
  Future<void> _cancel() async {
    ...
    if (error.isEmpty) {
      Position? position = (await LocationJunny().getCurrentLocation())!;
      if (position != null) {
        setState(() {
          _isSubmitting = true;
        });
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 5),
            _sufixLabel("Ingresa un motivo o describe brevemente la razón por la cual realiza la eliminación"),
            _textField(),
            Visibility(
                visible: error != '',
                child: Text(
...
void showDeclineTrasnfersProduct({required BuildContext context, required Function(String) onReject}) {
  showDialog(
    context: context,
    ...
    builder: (_) => AlertDialog(
        ...
        content: DeclineTrasnfersProduct(onReject: onReject)),
  );
}
